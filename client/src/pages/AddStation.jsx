...
class AddStation extends React.Component{
    ...
    checkInput = ()=>{
        ...
        if(title.length < 5){
            this.showMessage("عنوان نامعتبر");
            return false;
        }

        if(short_info.length < 8){
            this.showMessage("توضیح مختصر نامعتبر");
            return false;
        }

        if(xp < 80 || xp>240){
            this.showMessage("مقدار امتیاز نامعتبر است");
            return false;
        }

        if(info.length < 300 || info.length > 2000){
            this.showMessage("توضیحات باید در حد یک پاراگراف باشد");
            return false;
        }

        if(Number(location.lat) < 37 || Number(location.lat) > 38 || location.lat.length != 7){
            this.showMessage("مقدار عرض جغرافیایی با 4 رقم اعشار باید در حیطه رشت باشد");
            return false;
        }

        if(Number(location.lng) < 49 || Number(location.lng) > 50 || location.lat.length != 7){
            this.showMessage("مقدار طول جغرافیایی با 4 رقم اعشار باید در حیطه رشت باشد");
            return false;
        }

        if(!this.state.pic[0].file){
            this.showMessage("ایستگاه باید حداقل یک عکس داشته باشد");
            return false;
        }
